...
class QuietanzaVoter extends Voter {
	...
	protected function supports($attribute, $subject) {
		...
		// only vote on QuietanzaGiustificativo objects inside this voter
		if (!$subject instanceof QuietanzaGiustificativo) {
			return false;
		}
	...
	private function canRead(QuietanzaGiustificativo $quietanza, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($quietanza->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
		if (in_array('ROLE_UTENTE', $user->getRoles())) {
			$permesso = $this->findPermessoRichiesta($quietanza->getRichiesta(), $user);
			return $permesso;
		}
	}

	private function canWrite(QuietanzaGiustificativo $quietanza, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($quietanza->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
