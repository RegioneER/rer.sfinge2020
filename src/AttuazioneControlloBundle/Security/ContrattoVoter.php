...
class ContrattoVoter extends Voter {
	...
	protected function supports($attribute, $subject) {
		...
		// only vote on Contratto objects inside this voter
		if (!$subject instanceof Contratto) {
			return false;
		}
	...
	private function canRead(Contratto $contratto, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($contratto->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
		if (in_array('ROLE_UTENTE', $user->getRoles())) {
			$permesso = $this->findPermessoRichiesta($contratto->getRichiesta(), $user);
			return $permesso;
		}
	}

	private function canWrite(Contratto $contratto, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($contratto->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
