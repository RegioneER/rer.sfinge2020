...
class IntegrazionePagamentoVoter extends Voter {
	...
	protected function supports($attribute, $subject) {
		...
		if (!$subject instanceof IntegrazionePagamento) {
			return false;
		}
	...
	private function canRead(IntegrazionePagamento $integrazione, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($integrazione->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
		if (in_array('ROLE_UTENTE', $user->getRoles())) {
			$permesso = $this->findPermessoRichiesta($integrazione->getRichiesta(), $user);
			return $permesso;
		}
	}

	private function canWrite(IntegrazionePagamento $integrazione, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($integrazione->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
