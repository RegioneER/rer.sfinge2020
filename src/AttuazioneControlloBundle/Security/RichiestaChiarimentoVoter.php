...
class RichiestaChiarimentoVoter extends Voter {
	...
	protected function supports($attribute, $subject) {
		...
		// only vote on RichiestaChiarimento objects inside this voter
		if (!$subject instanceof RichiestaChiarimento) {
			return false;
		}
	...
	private function canRead(RichiestaChiarimento $richChiarimenti, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($richChiarimenti->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
		if (in_array('ROLE_UTENTE', $user->getRoles())) {
			$permesso = $this->findPermessoRichiesta($richChiarimenti->getRichiesta(), $user);
			return $permesso;
		}
	}

	private function canWrite(RichiestaChiarimento $richChiarimenti, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($richChiarimenti->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
