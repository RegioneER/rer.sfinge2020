...
class VocePianoCostoGiustificativoVoter extends Voter {
	...
	protected function supports($attribute, $subject) {
		...
		// only vote on VocePianoCostoGiustificativo objects inside this voter
		if (!$subject instanceof VocePianoCostoGiustificativo) {
			return false;
		}
	...
	private function canRead(VocePianoCostoGiustificativo $voce, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($voce->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
		if (in_array('ROLE_UTENTE', $user->getRoles())) {
			$permesso = $this->findPermessoRichiesta($voce->getRichiesta(), $user);
			return $permesso;
		}
	}

	private function canWrite(VocePianoCostoGiustificativo $voce, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($voce->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
