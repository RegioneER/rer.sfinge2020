...
class GiustificativoVoter extends Voter {
	...
	protected function supports($attribute, $subject) {
		...
		// only vote on GiustificativoPagamento objects inside this voter
		if (!$subject instanceof GiustificativoPagamento) {
			return false;
		}
	...
	private function canRead(GiustificativoPagamento $giustificativo, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($giustificativo->getPagamento()->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
		if (in_array('ROLE_UTENTE', $user->getRoles())) {
			$permesso = $this->findPermessoRichiesta($giustificativo->getPagamento()->getRichiesta(), $user);
			return $permesso;
		}
	}

	private function canWrite(GiustificativoPagamento $giustificativo, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($giustificativo->getPagamento()->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
