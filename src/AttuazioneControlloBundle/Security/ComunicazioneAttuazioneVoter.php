...
class ComunicazioneAttuazioneVoter extends Voter {
	...
	protected function supports($attribute, $subject) {
		...
		if (!$subject instanceof ComunicazioneAttuazione) {
			return false;
		}
	...
	private function canRead(ComunicazioneAttuazione $comunicazione, Utente $user) {
	...
	private function canWrite(ComunicazioneAttuazione $comunicazione, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($comunicazione->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
