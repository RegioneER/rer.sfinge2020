...
use AttuazioneControlloBundle\Entity\RispostaComunicazioneAttuazione;
...
class RispostaComunicazioneAttuazioneVoter extends Voter {
	...
	protected function supports($attribute, $subject) {
		...
		if (!$subject instanceof RispostaComunicazioneAttuazione) {
			return false;
		}
	...
	private function canRead(RispostaComunicazioneAttuazione $comunicazione, Utente $user) {
	...
	private function canWrite(RispostaComunicazioneAttuazione $comunicazione, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($comunicazione->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
