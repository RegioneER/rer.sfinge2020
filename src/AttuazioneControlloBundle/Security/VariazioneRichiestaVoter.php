...
class VariazioneRichiestaVoter extends Voter {
	...
	protected function supports($attribute, $subject) {
		...
		// only vote on VariazioneRichiesta objects inside this voter
		if (!$subject instanceof VariazioneRichiesta) {
			return false;
		}
	...
	private function canRead(VariazioneRichiesta $variazione, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($variazione->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
		if (in_array('ROLE_UTENTE', $user->getRoles())) {
			$permesso = $this->findPermessoRichiesta($variazione->getRichiesta(), $user);
			return $permesso;
		}
	}

	private function canWrite(VariazioneRichiesta $variazione, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($variazione->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
