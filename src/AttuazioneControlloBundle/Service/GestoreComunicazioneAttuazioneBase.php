 ...
 * @author aturdo
 */
class GestoreComunicazioneAttuazioneBase extends \BaseBundle\Service\BaseService {
    ...
    public function calcolaAzioniAmmesse($risposta_comunicazione_istruttoria) {
        ...
        if ($stato == StatoComunicazioneProgetto::COM_INSERITA && $this->isBeneficiario()) {
            ...
            $voceMenu["path"] = $this->generateUrl("risposta_comunicazione_attuazione_firmatario", array("id_comunicazione" => $comunicazione->getId()));
            $vociMenu[] = $voceMenu;
            ...
            if ($esitoValidazione->getEsito()) {
                ...
                $voceMenu["path"] = $this->generateUrl("valida_comunicazione_attuazione_risposta", array("id_comunicazione_risposta" => $risposta_comunicazione_istruttoria->getId(), "_token" => $token));
                $vociMenu[] = $voceMenu;
            }
        }
        ...
        if ($stato != StatoComunicazioneProgetto::COM_INSERITA) {
            ...
            $voceMenu["path"] = $this->generateUrl("scarica_comunicazione_attuazione_risposta", array("id_comunicazione_risposta" => $risposta_comunicazione_istruttoria->getId()));
            $vociMenu[] = $voceMenu;
        }

        //carica richiesta firmata
        if ($stato == StatoComunicazioneProgetto::COM_VALIDATA && $this->isBeneficiario()) {
            $voceMenu["label"] = "Carica risposta firmata";
            $voceMenu["path"] = $this->generateUrl("carica_comunicazione_risposta_attuazione_firmata", array("id_comunicazione_risposta" => $risposta_comunicazione_istruttoria->getId(), "id_comunicazione" => $comunicazione->getId()));
            $vociMenu[] = $voceMenu;
        }
        ...
        if ($stato == StatoComunicazioneProgetto::COM_FIRMATA && $this->isBeneficiario()) {
            ...
            $voceMenu["path"] = $this->generateUrl("invia_risposta_comunicazione_attuazione", array("id_comunicazione_risposta" => $risposta_comunicazione_istruttoria->getId(), "_token" => $token));
        ...
        if (($stato == StatoComunicazioneProgetto::COM_VALIDATA || $stato == StatoComunicazioneProgetto::COM_FIRMATA) && $this->isBeneficiario()) {
            ...
            $voceMenu["path"] = $this->generateUrl("invalida_comunicazione_risposta_attuazione", array("id_comunicazione_risposta" => $risposta_comunicazione_istruttoria->getId(), "_token" => $token));
    ...
    public function notaRispostaComunicazione($comunicazione, $opzioni) {
        ...
        $form_options['data_class'] = 'AttuazioneControlloBundle\Entity\RispostaComunicazioneAttuazione';

        $form = $this->createForm("AttuazioneControlloBundle\Form\NotaRispostaType", $comunicazione->getRisposta(), $form_options);
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid()) {
                ...
                try {
                    $em->flush();
                    $this->addFlash("success", "Nota salvata correttamente");
    ...
    public function elencoDocumenti($comunicazione, $proponente = null, $opzioni = array()) {
        ...
        $documento_comunicazione = new \AttuazioneControlloBundle\Entity\RispostaComunicazioneAttuazioneDocumento();
        $documento_file = new DocumentoFile();

        $documenti_caricati = $em->getRepository("AttuazioneControlloBundle\Entity\RispostaComunicazioneAttuazioneDocumento")->findBy(array("risposta_comunicazione" => $comunicazione->getRisposta(), "proponente" => $proponente));
        ...
        $response = $this->render("AttuazioneControlloBundle:RispostaComunicazioneAttuazione:elencoDocumentiRichiesta.html.twig", $dati);
        return new GestoreResponse($response);
    }
    ...
    public function sceltaFirmatario($comunicazione, $opzioni = array()) {
        ...
        $form = $this->createForm("AttuazioneControlloBundle\Form\SceltaFirmatarioType", $comunicazione->getRisposta(), $form_options);
        ...
        $response = $this->render("AttuazioneControlloBundle:RispostaComunicazioneAttuazione:sceltaFirmatario.html.twig", $dati);
    ...
    public function eliminaDocumentoComunicazioneAttuazione($documento, $opzioni = array()) {
        $em = $this->getEm();
        try {
    ...
    public function eliminaDocumentoComunicazioneRisposta($id_comunicazione, $opzioni = array()) {
        $em = $this->getEm();
        $documento = $em->getRepository("AttuazioneControlloBundle\Entity\RispostaComunicazioneAttuazioneDocumento")->find($id_comunicazione);
        ...
        if ($comunicazione_risposta->getStato()->uguale(StatoComunicazioneProgetto::COM_FIRMATA)) {
            try {
                ...
                if ($this->container->getParameter("stacca_protocollo_al_volo")) {
                    $this->container->get("docerinitprotocollazione")->setTabProtocollazioneRispostaComunicazioneAttuazione($comunicazione_risposta);
                }
                $this->getEm()->flush();
                $this->getEm()->commit();
            } catch (\Exception $ex) {
    ...
    public function creaComunicazione($oggetto, $tipo_oggetto) {

        $em = $this->getEm();
        $comunicazione = new \AttuazioneControlloBundle\Entity\ComunicazioneAttuazione();
        try {

            if ($tipo_oggetto == 'ATTUAZIONE') {
                $richiesta = $oggetto;
                $comunicazione->setRichiesta($richiesta);
            }
        ...
        $response = new GestoreResponse($this->generaUrlDaTipo($oggetto, $tipo_oggetto));
        return $response->getResponse();
    }

    public function gestioneComunicazioneAttuazione($comunicazione) {

        $em = $this->getEm();

        if ($comunicazione->getTipoOggetto() == 'ATTUAZIONE') {
            $richiesta = $comunicazione->getRichiesta();
            $indietro = $this->generateUrl("elenco_comunicazioni_attuazione", array("id_richiesta" => $richiesta->getId()));
        }

        $documento_comunicazione = new \AttuazioneControlloBundle\Entity\ComunicazioneAttuazioneDocumento();
        ...
        if (count($listaTipi) > 0) {
            ...
            $form_documenti = $this->createForm('AttuazioneControlloBundle\Form\Istruttoria\ComunicazioneAttuazioneDocumentoType', $documento_comunicazione, $opzioni_form_documenti);

            $opzioni_form_esito["url_indietro"] = $indietro;
            $opzioni_form_esito["disabled"] = $disabilita_azioni;
            $form_esito = $this->createForm('AttuazioneControlloBundle\Form\Istruttoria\ComunicazioneAttuazioneType', $comunicazione, $opzioni_form_esito);
            ...
            if ($request->isMethod('POST')) {
                ...
                if ($form_documenti->isSubmitted() && $form_documenti->isValid()) {
                    try {
                        ...
                        return new GestoreResponse($this->redirect($this->generateUrl('gestione_comunicazione_attuazione', array("id_comunicazione" => $comunicazione->getId()))));
                    } catch (ResponseException $e) {
                ...
                if ($form_esito->isSubmitted() && $form_esito->isValid()) {
                    ...
                    try {
                        ...
                        if ($form_esito->get("pulsanti")->get("pulsante_invio")->isClicked()) {
                            ...
                            $this->generaPdfComunicazioneAttuazione($comunicazione);
                            $em->flush();
                            ...
                            if ($this->container->getParameter("stacca_protocollo_al_volo")) {
                                ...
                                $richiestaProtocollo = $this->container->get("docerinitprotocollazione")->setTabProtocollazioneComunicazioneAttuazione($comunicazione);
                                $em->flush();
        ...
        if ($comunicazione->getTipoOggetto() == 'ATTUAZIONE') {
            ...
            $response = $this->render("AttuazioneControlloBundle:PA/Richieste:comunicazioneAttuazione.html.twig", $dati);
        } else {
            throw new \Exception('Non è stato definito un oggetto per il tipo: ' . $comunicazione->getTipoOggetto());
        }

        return new GestoreResponse($response);
    }

    public function generaFacsimileComunicazioneAttuazione($comunicazione) {
        ...
        $dati["is_fsc"] = $isFsc;
        $twig = "AttuazioneControlloBundle:PA/Richieste:pdfComunicazioneAttuazione.html.twig";
        $pdf->load($twig, $dati);
        ...
        if ($download) {
            $date = new \DateTime();
            $nome_file = "Comunicazione_attuazione_{$richiesta->getId()}_{$date->format('Y-m-d')}.pdf";
    ...
    public function generaPdfComunicazioneAttuazione($comunicazione) {
        $pdf = $this->container->get("pdf");
        ...
        $documentoRichiesta = $this->container->get("documenti")->caricaDaByteArray($data, "Comunicazione_attuazione_{$richiesta->getId()}_{$data_corrente->format('Y-m-d')}.pdf", $tipoDocumento);
    ...
    protected function generaUrlDaTipo($oggetto, $tipo_oggetto) {
        switch ($tipo_oggetto) {
            case 'ATTUAZIONE':
                return $this->redirectToRoute("elenco_comunicazioni_attuazione", array('id_richiesta' => $oggetto->getId()));
        }
    }
