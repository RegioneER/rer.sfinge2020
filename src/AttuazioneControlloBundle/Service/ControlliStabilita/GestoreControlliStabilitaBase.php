...
class GestoreControlliStabilitaBase extends AGestoreControlliStabilita {
    ...
    public function riepilogoControllo($controllo) {
        ...
        try {
            ...
            return $this->redirectToRoute("elenco_controlli_stabilita", array('id_campione' => $controllo->getCampione()->getId()));
        }

        $options = array();
        $options["disabled"] = !$this->isGranted("ROLE_ISTRUTTORE_CONTROLLI") || !is_null($controllo->getEsito());
        $options["url_indietro"] = $this->generateUrl('elenco_controlli_stabilita', array('id_campione' => $controllo->getCampione()->getId()));
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid()) {
                try {
                    ...
                    return $this->redirect($this->generateUrl('riepilogo_controllo_stabilita', array('id_controllo' => $controllo->getId())));
                } catch (\Exception $e) {
        ...
        return $this->render("AttuazioneControlloBundle:ControlliStabilita:riepilogoControllo.html.twig", $dati);
    }
    ...
    public function documentiControllo($controllo) {
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid()) {
                try {
                    ...
                    return $this->redirectToRoute("documenti_controlli_stabilita", array("id_controllo" => $controllo->getId()));
                } catch (\Exception $e) {
        ...
        return $this->render("AttuazioneControlloBundle:ControlliStabilita:documentiControllo.html.twig", $dati);
    }

    public function inizializzaControllo($controllo) {
        $tipoCL = $this->dammiClDaControllo($controllo);
        if (is_null($controllo->getValutazioniChecklist()) || $controllo->getValutazioniChecklist()->count() == 0) {
            $checklists = $this->getEm()->getRepository("AttuazioneControlloBundle\Entity\Controlli\ChecklistControllo")->findByCodice(array($tipoCL));
    ...
    public function valutaChecklist($valutazione_checklist, $extra = array()) {
        ...
        $options = array();
        $options["url_indietro"] = $this->generateUrl('riepilogo_controllo_stabilita', array("id_controllo" => $controllo->getId()));
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid()) {
                ...
                if ($options["valida"] && $form->get("pulsanti")->get("pulsante_valida")->isClicked()) {
                    ...
                    $redirect_url = $this->generateUrl('valuta_checklist_controlli_stabilita', array('id_valutazione_checklist' => $valutazione_checklist->getId()));
                } else {
        ...
        $dati["form"] = $form->createView();
        $dati["controllo"] = $controllo;
        $dati["procedura"] = $controllo->getCampione()->getId();
        ...
        $twig = isset($extra["twig"]) ? $extra["twig"] : "AttuazioneControlloBundle:ControlliStabilita:checklistControllo.html.twig";
    ...
    public function esitoFinale($controllo) {
        ...
        $form = $this->createForm("AttuazioneControlloBundle\Form\ControlliStabilita\EsitoControlloType", $controllo, $options);
            ...
            if ($form->isValid()) {
                ...
                if ($controllo->getEsito()) {
                    ...
                    if (!$esito->getEsito()) {
                        ...
                        return $this->redirect($this->generateUrl('esito_finale_controlli_stabilita', array('id_controllo' => $controllo->getId())));
                    }
                }
                ...
                try {
                    ...
                    return $this->redirect($this->generateUrl("elenco_controlli_stabilita", array('id_campione' => $controllo->getCampione()->getId())));
                } catch (\Exception $e) {
                    $this->addFlash('error', "Errore nel salvataggio delle informazioni");
                }
            }
        ...
        $twig = "AttuazioneControlloBundle:ControlliStabilita:esitoFinale.html.twig";
    ...
    public function validaChecklist($valutazione_checklist) {
        ...
        //il concetto di procedure quì è frainteso
        //non si intende la procedura operativa ma la procedura del campione quindi campione->getId()
        //questo perchè i campioni le cl sono personalizzate per campione e non per procedura
        //La colonna a db è la stessa perchè esistono cl con pesonalizzazioni per procedura operativa quindi va contestualizzata in base alla cl
        //Per i controlli ex-post la procedura è il campione e questo è il caso
        $procedura = $valutazione_checklist->getControlloProgetto()->getCampione()->getId();
    ...
    public function eliminaDocumentoControllo($controllo, $documento_controllo, $verbale) {
        ...
        try {
            ...
            $em->flush();
            if ($verbale == '1') {
                return $this->addSuccesRedirect("Il documento è stato correttamente eliminato", "valuta_sopralluogo_form_stabilita", array("id_controllo" => $controllo->getId()));
            } else {
                return $this->addSuccesRedirect("Il documento è stato correttamente eliminato", "documenti_controlli_stabilita", array("id_controllo" => $controllo->getId()));
            }
        } catch (ResponseException $e) {
            return $this->addErrorRedirect("Errore nel salvataggio delle informazioni. Si prega di riprovare o contattare l'assistenza.", "documenti_controlli_stabilita", array("id_controllo" => $controllo->getId()));
        }
    }
    ...
    public function valutaSopralluogoForm($controllo) {
        ...
        $opzioni_form_valutazione = array(
            'url_indietro' => $this->generateUrl('riepilogo_controllo_stabilita', array("id_controllo" => $controllo->getId())),
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form_doc->isSubmitted() && $form_doc->isValid()) {
                try {
                    ...
                    return $this->redirect($this->generateUrl('valuta_sopralluogo_form_stabilita', array("id_controllo" => $controllo->getId())));
                } catch (ResponseException $e) {
        ...
        return $this->render("AttuazioneControlloBundle:ControlliStabilita:valutazioneSopralluogoForm.html.twig", $dati);
    }
    ...
    public function estrazioneUniversoStabilita() {
        $campioni = $this->getEm()->getRepository("AttuazioneControlloBundle\Entity\Controlli\ControlloProgetto")->estrazioneUniversoStabilita();
        ...
        $column++;
        $activeSheet->setCellValueExplicitByColumnAndRow($column, $riga, 'pmi');
        $column++;
        ...
        $column++;
        $activeSheet->setCellValueExplicitByColumnAndRow($column, $riga, 'Data liquidazione');
        $column++;
        ...
        foreach ($campioni as $key => $campione) {
            ...
            $column++;
            $pmiRaw = $campione['pmi'];
            $pmiRes = ($pmiRaw != '-' ? ($pmiRaw == 'GRANDE' ? 'No':'Si'):'-');
            $activeSheet->setCellValueExplicitByColumnAndRow($column, $riga, $pmiRes);
            $column++;
            ...
            $column++;
            $activeSheet->setCellValueExplicitByColumnAndRow($column, $riga, $campione['data_liquidazione'] == '-' ? $campione['data_liquidazione'] : (new \DateTime($campione['data_liquidazione']))->format('d-m-Y'));
            $column++;
    ...
    public function verbaleSopralluogoControllo($controllo) {
        ...
        $options["url_indietro"] = $this->generateUrl('verbale_sopralluogo_form_stabilita', array('id_controllo' => $controllo->getId()));
        $options["dataIndirizzo"] = $data;
        $form = $this->createForm("AttuazioneControlloBundle\Form\ControlliStabilita\VerbaleSopralluogoControlloType", $controllo, $options);
        ...
        return $this->render("AttuazioneControlloBundle:ControlliStabilita:verbaleSopralluogoControllo.html.twig", $dati);
    }

    public function generaVerbaleSopralluogoStabilita($controllo) {
        $tipoCl = $this->dammiClDaControllo($controllo);
        $procedura = $controllo->getCampione();
        $checklist_spr = $this->getEm()->getRepository("AttuazioneControlloBundle\Entity\Controlli\ChecklistControllo")->findOneByCodice(array($tipoCl));

        $valutazioni = $controllo->getValutazioniChecklist();
        $valutatore = null;
        foreach ($valutazioni as $valutazione) {
            if ($valutazione->getChecklist()->getNome() == 'Check list') {
        ...
        $dati['twig'] = 'AttuazioneControlloBundle:ControlliStabilita:pdfVerbaleSopralluogoControllo.html.twig';
    ...
    public function estrazioneUniversoCampionario($campione) {
        $campioni = $this->getEm()->getRepository("AttuazioneControlloBundle\Entity\Controlli\ControlloCampione")->estrazioneUniversoStabilitaByCampione($campione);
        ...
        $activeSheet->setCellValueExplicitByColumnAndRow($column, $riga, 'pagamento');
        $column++;
        $activeSheet->setCellValueExplicitByColumnAndRow($column, $riga, 'data mandato pagamento');
        $column++;
        ...
        foreach ($campioni as $key => $campione) {
            ...
            $activeSheet->setCellValueExplicitByColumnAndRow($column, $riga, $campione['modalita_ultimo_pagamento']);
            $column++;
            $activeSheet->setCellValueExplicitByColumnAndRow($column, $riga, $campione['data_pagamento']);
            $column++;
    ...
    public function documentiControlloCampione($controllo) {
        $request = $this->getCurrentRequest();
        $em = $this->getEm();

        $documento_file = new \DocumentoBundle\Entity\DocumentoFile();
        $documento_controllo = new \AttuazioneControlloBundle\Entity\Controlli\DocumentoControlloCampione();
        $documento_controllo->setDocumentoFile($documento_file);
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid()) {
                try {
                    ...
                    $em->flush();
                    $this->addFlash('success', "Documento caricato correttamente");
                    return $this->redirectToRoute("documenti_controllo_campione", array("id_controllo" => $controllo->getId()));
                } catch (\Exception $e) {
                    $this->container->get("logger")->error($e->getMessage());
                    $this->addFlash('error', "Si è verificato un errore a sistema. Si prega di riprovare o contattare l'assistenza");
                }
        ...
        return $this->render("AttuazioneControlloBundle:ControlliStabilita:documentiControlloCampione.html.twig", $dati);
    }

    public function eliminaDocumentoControlloCampione($controllo, $documento_controllo) {
        $em = $this->getEm();
        try {
            $em->remove($documento_controllo->getDocumentoFile());
            $em->remove($documento_controllo);
            $em->flush();
            return $this->addSuccesRedirect("Il documento è stato correttamente eliminato", "documenti_controllo_campione", array("id_controllo" => $controllo->getId()));
        } catch (ResponseException $e) {
            return $this->addErrorRedirect("Errore nel salvataggio delle informazioni. Si prega di riprovare o contattare l'assistenza.", "documenti_controllo_campione", array("id_controllo" => $controllo->getId()));
        }
    }

    public function dammiClDaControllo($controllo) {
        $tipo = $controllo->getTipologia();
        switch ($tipo) {
            case 'STABILITA':
                $tipoOut = 'CHECK_SPR_STABILITA';
                break;
            case 'PUNTUALE':
                $tipoOut = 'CHECK_PNT_STABILITA';
                break;
            default:
                $tipoOut = 'CHECK_SPR_STABILITA';
                break;
