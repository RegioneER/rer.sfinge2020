 ...
 */
class IntegrazioneDocERService extends DocERBaseService {
	 ...
	 * @var DocERProtocollazioneService
	 */
	protected $DocERProtocollazioneService;

	function getDocERProtocollazioneService() {
		return $this->DocERProtocollazioneService;
	}

	function setDocERProtocollazioneService(DocERProtocollazioneService $DocERProtocollazioneService) {
		$this->DocERProtocollazioneService = $DocERProtocollazioneService;
	}
	...
	public function initServizi() {
		try {
			...
			$this->initDocERServizio("DocERProtocollazioneService");
		} catch (\Exception $ex) {
			throw $ex;
		}
	}
...
//			$as = substr($docER_token, 0, 255); per debug
	...
	public function initProtocollazione(
            $Protocollazione_Oggetto, $Protocollazione_Mittente_Persona_Giuridica_id, 
            $Protocollazione_Mittente_Persona_Giuridica_Denominazione, $Protocollazione_Fascicolo_primario_Progressivo, $Protocollazione_Registro_Id, 
            $Protocollazione_CodiceAOO = null, $Protocollazione_CodiceAmministrazione = null, $Protocollazione_Identificativo = null, 
            $Protocollazione_Classifica = null, $Protocollazione_Fascicolo_primario_Anno = null
	) {
        ...
        $this->Protocollazione_Registro_Id = (\is_null($Protocollazione_Registro_Id)) ? 'PG' : $Protocollazione_Registro_Id;
	}
	...
	public function makeXmlProtocolloAzienda() {
		...
		try {
			$XmlBuilderProtocollo = $this->getDocERProtocollazioneService()->getBuilderXmlProtocollo();
	...
	public function protocollaUnitaDocumentale() {
		...
		try {
			...
			$st = $this->getDocERProtocollazioneService()->protocollaById($documentoId, $datiProtocollo, $this->Protocollazione_Registro_Id );
			if ($st === false) {
	...
	public function protocollaUnitaDocumentaleInUscita($param, $registro_id) {
		...
		try {
			...
			$st = $this->getDocERProtocollazioneService()->protocollaById($documentoId, $datiProtocollo, $registro_id);
			if ($st === false) {
				throw new \Exception("Avviso: nessun dato di protocollazione restituito da DocER");
			}
