...
/**
 * Description of inviaEmailProtocolloCommand
 *
 * @author gdisparti
 */
class inviaEmailProtocolloCommand extends ContainerAwareCommand {

	protected function configure() {
		$this->setName('egrammata:inviaEmailProtocollo')->setDescription('');
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$output->writeln("<comment>********** Inizio procedura di invio email protocollazione **********</comment>");
		...
		do {
			...
			$emailProtocolloDaInviare = $repo->getEmailProtocolloDaInviare($lotto, $ids);
			
			foreach ($emailProtocolloDaInviare as $emailProtocollo) {
				...
				try {
					...
					$resp = $egrammata->inviaEmail($emailProtocollo);
					if($resp){
						$emailProcessate_OK++;
					}else{
						$emailProcessate_KO++;	
					...
					$output->writeln("<comment>Eccezione durante la procedura di invio EmailProtocollo {$emailProtocollo->getId()}: ".$e->getMessage()."</comment>");
				}
		...
		} while (count($emailProtocolloDaInviare) > 0 && !$memoria_esaurita && $numeroIterazione <= $numeroMassimoIterazioni);
		...
		$output->writeln("<comment>********** Procedura di invio email protocollazione completata **********</comment>");
	}
