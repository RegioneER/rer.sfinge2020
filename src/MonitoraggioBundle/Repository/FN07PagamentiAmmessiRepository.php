...
class FN07PagamentiAmmessiRepository extends EsportazioneProgettoRepository {
    public function findAllFiltered(\MonitoraggioBundle\Form\Entity\Strutture\FN07 $ricerca) {
        $q = $this->getEntityManager()->createQuery();
        $query = 'select e '
                . 'from MonitoraggioBundle:FN07PagamentiAmmessi e '
                ...
                . "and coalesce( e.importo_pag_amm, 0) = coalesce( :importo_pag_amm, e.importo_pag_amm, 0) "
                . "order by e.id asc";
        $q->setDQL($query);
        ...
        $q->setParameter(':importo_pag_amm', $ricerca->getImportoPagAmm());
    ...
    public function isEsportabile(MonitoraggioConfigurazioneEsportazioneRichiesta $struttura): bool {
        ...
        ->join('pagamenti.pagamenti_ammessi', 'pagamenti_ammessi')
        ->andWhere(
            $expr->gte("coalesce(struttura.data_modifica, struttura.data_creazione,'0000-00-00')",
            "coalesce(pagamenti_ammessi.data_modifica, pagamenti_ammessi.data_creazione,'0000-00-00')")
        )
        ->getQuery()
        ->getSingleResult();
    ...
    public function findAllEsportabili($date) {
        $query = 'select distinct r '
                ...
                . 'join mon_richieste_pagamento.pagamenti_ammessi pagamenti_ammessi '
                ...
                . "or coalesce(pagamenti_ammessi.data_modifica, pagamenti_ammessi.data_creazione) > :data_esportazione) AND COALESCE(r.flag_por, 0) = 1 AND COALESCE(r.flag_inviato_monit, 0) = 0";
