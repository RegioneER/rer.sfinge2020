...
class ComunicazioneProgettoVoter extends Voter {
	...
	protected function supports($attribute, $subject) {
		...
		if (!$subject instanceof ComunicazioneProgetto) {
			return false;
		}
	...
	private function canRead(ComunicazioneProgetto $comunicazione, Utente $user) {
	...
	private function canWrite(ComunicazioneProgetto $comunicazione, Utente $user) {
		if (in_array('ROLE_UTENTE_PA', $user->getRoles())) {
			$permesso = $this->findPermesso($comunicazione->getRichiesta()->getProcedura(), $user);
			return !is_null($permesso);
		}
