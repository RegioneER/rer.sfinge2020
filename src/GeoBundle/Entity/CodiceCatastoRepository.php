...
class CodiceCatastoRepository extends EntityRepository
{
    ...
    public function getGeoByCodiceCatasto($codice_catasto) {
        ...
        try {
            ...
            if (\strlen($codice_istat->getCodiceIstat()) > 5) {
                $query = "SELECT c FROM GeoBundle:GeoComune c WHERE c.codice_completo='".$codice_istat->getCodiceIstat()."'";      
            } else {
                $query = "SELECT s FROM GeoBundle:GeoStato s WHERE s.codice='".$codice_istat->getCodiceIstat()."'";
            }
	...
	public function findCatastoByIstat($codice_istat) {
        return $this->findOneBy(array('codice_istat' => $codice_istat));
    }
    
    public function findCatasto($codice) {
        
         $query= "SELECT cc.codice_catasto
                FROM GeoBundle:CodiceCatasto cc
                WHERE cc.codice_istat=".$codice;
